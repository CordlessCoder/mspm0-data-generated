...
pub mod regs {
    ...
    impl Genclkcfg {
        ...
        ///FCCLFCLKSRC selects between SYSTEM LFCLK and EXTERNAL SOURCED LFCLK.
        #[must_use]
        #[inline(always)]
    ...
    impl defmt::Format for Genclkcfg {
        fn format(&self, f: defmt::Formatter) {
            defmt::write!(
                f,
                "Genclkcfg {{ exclksrc: {:?}, exclkdivval: {:?}, exclkdiven: {=bool:?}, mfpclksrc: {:?}, hfclk4mfpclkdiv: {:?}, fccselclk: {:?}, fcctrigsrc: {:?}, fcclvltrig: {:?}, anacpumpcfg: {:?}, fcctrigcnt: {=u8:?}, fcclfclksrc: {=bool:?} }}",
                self.exclksrc(), self.exclkdivval(), self.exclkdiven(), self.mfpclksrc(),
                self.hfclk4mfpclkdiv(), self.fccselclk(), self.fcctrigsrc(), self
                .fcclvltrig(), self.anacpumpcfg(), self.fcctrigcnt(), self.fcclfclksrc()
            )
        }
    }
    ///General clock enable control.
    ...
    impl defmt::Format for Int {
        fn format(&self, f: defmt::Formatter) {
            defmt::write!(
                f,
                "Int {{ lfoscgood: {=bool:?}, anaclkerr: {=bool:?}, lfxtgood: {=bool:?}, hfclkgood: {=bool:?}, hsclkgood: {=bool:?} }}",
                self.lfoscgood(), self.anaclkerr(), self.lfxtgood(), self.hfclkgood(),
                self.hsclkgood()
            )
        }
    }
    ...
    impl defmt::Format for Nmi {
        fn format(&self, f: defmt::Formatter) {
            defmt::write!(
                f, "Nmi {{ borlvl: {=bool:?}, wwdt0: {=bool:?}, lfclkfail: {=bool:?} }}",
                self.borlvl(), self.wwdt0(), self.lfclkfail()
            )
        }
    }
    #[repr(transparent)]
