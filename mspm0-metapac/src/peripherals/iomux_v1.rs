// This file is autogenerated by mspm0-metapac-gen: do not edit by hand.
#![allow(clippy::missing_safety_doc)]
#![allow(clippy::identity_op)]
#![allow(clippy::unnecessary_cast)]
#![allow(clippy::erasing_op)]
///PERIPHERALREGION.
#[derive(Copy, Clone, Eq, PartialEq)]
pub struct Iomux {
    ptr: *mut u8,
}
unsafe impl Send for Iomux {}
unsafe impl Sync for Iomux {}
impl Iomux {
    #[inline(always)]
    pub const unsafe fn from_ptr(ptr: *mut ()) -> Self {
        Self { ptr: ptr as _ }
    }
    #[inline(always)]
    pub const fn as_ptr(&self) -> *mut () {
        self.ptr as _
    }
    ///Pin Control Management Register in SECCFG region. The array is sized for the largest device, so the caller is responsible for checking that an entry exists on the chip in hand before accessing it.
    #[inline(always)]
    pub const fn pincm(
        self,
        n: usize,
    ) -> crate::common::Reg<regs::Pincm, crate::common::RW> {
        assert!(n < 251usize);
        unsafe {
            crate::common::Reg::from_ptr(
                self.ptr.wrapping_add(0x04usize + n * 4usize) as _,
            )
        }
    }
}
pub mod regs {
    ///Pin Control Management Register in SECCFG region.
    #[repr(transparent)]
    #[derive(Copy, Clone, Eq, PartialEq)]
    pub struct Pincm(pub u32);
    impl Pincm {
        ///Peripheral Function selection bits.
        #[must_use]
        #[inline(always)]
        pub const fn pf(&self) -> u8 {
            let val = (self.0 >> 0usize) & 0x3f;
            val as u8
        }
        ///Peripheral Function selection bits.
        #[inline(always)]
        pub const fn set_pf(&mut self, val: u8) {
            self.0 = (self.0 & !(0x3f << 0usize)) | (((val as u32) & 0x3f) << 0usize);
        }
        ///Peripheral is Connected.
        #[must_use]
        #[inline(always)]
        pub const fn pc(&self) -> bool {
            let val = (self.0 >> 7usize) & 0x01;
            val != 0
        }
        ///Peripheral is Connected.
        #[inline(always)]
        pub const fn set_pc(&mut self, val: bool) {
            self.0 = (self.0 & !(0x01 << 7usize)) | (((val as u32) & 0x01) << 7usize);
        }
        ///This has the IOPAD WAKEUP signal as status bit.
        #[must_use]
        #[inline(always)]
        pub const fn wakestat(&self) -> bool {
            let val = (self.0 >> 13usize) & 0x01;
            val != 0
        }
        ///This has the IOPAD WAKEUP signal as status bit.
        #[inline(always)]
        pub const fn set_wakestat(&mut self, val: bool) {
            self.0 = (self.0 & !(0x01 << 13usize)) | (((val as u32) & 0x01) << 13usize);
        }
        ///Pull Down control selection.
        #[must_use]
        #[inline(always)]
        pub const fn pipd(&self) -> bool {
            let val = (self.0 >> 16usize) & 0x01;
            val != 0
        }
        ///Pull Down control selection.
        #[inline(always)]
        pub const fn set_pipd(&mut self, val: bool) {
            self.0 = (self.0 & !(0x01 << 16usize)) | (((val as u32) & 0x01) << 16usize);
        }
        ///Pull Up control selection.
        #[must_use]
        #[inline(always)]
        pub const fn pipu(&self) -> bool {
            let val = (self.0 >> 17usize) & 0x01;
            val != 0
        }
        ///Pull Up control selection.
        #[inline(always)]
        pub const fn set_pipu(&mut self, val: bool) {
            self.0 = (self.0 & !(0x01 << 17usize)) | (((val as u32) & 0x01) << 17usize);
        }
        ///Input Enable Control Selection.
        #[must_use]
        #[inline(always)]
        pub const fn inena(&self) -> bool {
            let val = (self.0 >> 18usize) & 0x01;
            val != 0
        }
        ///Input Enable Control Selection.
        #[inline(always)]
        pub const fn set_inena(&mut self, val: bool) {
            self.0 = (self.0 & !(0x01 << 18usize)) | (((val as u32) & 0x01) << 18usize);
        }
        ///Hysteresis Enable Control Selection.
        #[must_use]
        #[inline(always)]
        pub const fn hysten(&self) -> bool {
            let val = (self.0 >> 19usize) & 0x01;
            val != 0
        }
        ///Hysteresis Enable Control Selection.
        #[inline(always)]
        pub const fn set_hysten(&mut self, val: bool) {
            self.0 = (self.0 & !(0x01 << 19usize)) | (((val as u32) & 0x01) << 19usize);
        }
        ///Drive strength control selection, for HS IOCELL only.
        #[must_use]
        #[inline(always)]
        pub const fn drv(&self) -> bool {
            let val = (self.0 >> 20usize) & 0x01;
            val != 0
        }
        ///Drive strength control selection, for HS IOCELL only.
        #[inline(always)]
        pub const fn set_drv(&mut self, val: bool) {
            self.0 = (self.0 & !(0x01 << 20usize)) | (((val as u32) & 0x01) << 20usize);
        }
        ///High output value will tri-state the output when this bit is enabled.
        #[must_use]
        #[inline(always)]
        pub const fn hiz1(&self) -> bool {
            let val = (self.0 >> 25usize) & 0x01;
            val != 0
        }
        ///High output value will tri-state the output when this bit is enabled.
        #[inline(always)]
        pub const fn set_hiz1(&mut self, val: bool) {
            self.0 = (self.0 & !(0x01 << 25usize)) | (((val as u32) & 0x01) << 25usize);
        }
        ///Data inversion selection.
        #[must_use]
        #[inline(always)]
        pub const fn inv(&self) -> bool {
            let val = (self.0 >> 26usize) & 0x01;
            val != 0
        }
        ///Data inversion selection.
        #[inline(always)]
        pub const fn set_inv(&mut self, val: bool) {
            self.0 = (self.0 & !(0x01 << 26usize)) | (((val as u32) & 0x01) << 26usize);
        }
        ///Wakeup Enable bit.
        #[must_use]
        #[inline(always)]
        pub const fn wuen(&self) -> bool {
            let val = (self.0 >> 27usize) & 0x01;
            val != 0
        }
        ///Wakeup Enable bit.
        #[inline(always)]
        pub const fn set_wuen(&mut self, val: bool) {
            self.0 = (self.0 & !(0x01 << 27usize)) | (((val as u32) & 0x01) << 27usize);
        }
        ///Wakeup Compare Value bit.
        #[must_use]
        #[inline(always)]
        pub const fn wcomp(&self) -> bool {
            let val = (self.0 >> 28usize) & 0x01;
            val != 0
        }
        ///Wakeup Compare Value bit.
        #[inline(always)]
        pub const fn set_wcomp(&mut self, val: bool) {
            self.0 = (self.0 & !(0x01 << 28usize)) | (((val as u32) & 0x01) << 28usize);
        }
    }
    impl Default for Pincm {
        #[inline(always)]
        fn default() -> Pincm {
            Pincm(0)
        }
    }
    impl core::fmt::Debug for Pincm {
        fn fmt(&self, f: &mut core::fmt::Formatter) -> core::fmt::Result {
            f.debug_struct("Pincm")
                .field("pf", &self.pf())
                .field("pc", &self.pc())
                .field("wakestat", &self.wakestat())
                .field("pipd", &self.pipd())
                .field("pipu", &self.pipu())
                .field("inena", &self.inena())
                .field("hysten", &self.hysten())
                .field("drv", &self.drv())
                .field("hiz1", &self.hiz1())
                .field("inv", &self.inv())
                .field("wuen", &self.wuen())
                .field("wcomp", &self.wcomp())
                .finish()
        }
    }
    #[cfg(feature = "defmt")]
    impl defmt::Format for Pincm {
        fn format(&self, f: defmt::Formatter) {
            defmt::write!(
                f,
                "Pincm {{ pf: {=u8:?}, pc: {=bool:?}, wakestat: {=bool:?}, pipd: {=bool:?}, pipu: {=bool:?}, inena: {=bool:?}, hysten: {=bool:?}, drv: {=bool:?}, hiz1: {=bool:?}, inv: {=bool:?}, wuen: {=bool:?}, wcomp: {=bool:?} }}",
                self.pf(), self.pc(), self.wakestat(), self.pipd(), self.pipu(), self
                .inena(), self.hysten(), self.drv(), self.hiz1(), self.inv(), self
                .wuen(), self.wcomp()
            )
        }
    }
}
