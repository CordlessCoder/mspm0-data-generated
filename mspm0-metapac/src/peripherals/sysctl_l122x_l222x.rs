...
pub mod regs {
    ...
    impl defmt::Format for Clkstatus {
        fn format(&self, f: defmt::Formatter) {
            defmt::write!(
                f,
                "Clkstatus {{ sysoscfreq: {:?}, hsclkmux: {:?}, lfclkmux: {:?}, hfclkgood: {=bool:?}, lfxtgood: {=bool:?}, lfoscgood: {=bool:?}, hsclksoff: {=bool:?}, hfclkoff: {=bool:?}, curhsclksel: {:?}, curmclksel: {=bool:?}, hsclkdead: {=bool:?}, hsclkgood: {=bool:?}, lfclkfail: {=bool:?}, fclmode: {=bool:?}, fccdone: {=bool:?}, hfclkblkupd: {=bool:?}, anaclkerr: {=bool:?} }}",
                self.sysoscfreq(), self.hsclkmux(), self.lfclkmux(), self.hfclkgood(),
                self.lfxtgood(), self.lfoscgood(), self.hsclksoff(), self.hfclkoff(),
                self.curhsclksel(), self.curmclksel(), self.hsclkdead(), self
                .hsclkgood(), self.lfclkfail(), self.fclmode(), self.fccdone(), self
                .hfclkblkupd(), self.anaclkerr()
            )
        }
    }
    ///LFCLK_IN and LFCLK control.
    ...
    impl defmt::Format for Genclkcfg {
        fn format(&self, f: defmt::Formatter) {
            defmt::write!(
                f,
                "Genclkcfg {{ exclksrc: {:?}, exclkdivval: {:?}, exclkdiven: {=bool:?}, mfpclksrc: {:?}, hfclk4mfpclkdiv: {:?}, fccselclk: {:?}, fcctrigsrc: {:?}, fcclvltrig: {:?}, anacpumpcfg: {:?}, fcctrigcnt: {=u8:?} }}",
    ...
    impl Hsclkcfg {
        ///Selects the HSCLK source. This device has no SYSPLL, so HFCLKCLK is the only source it can select, and software must write it - the reset value leaves HSCLK unsourced.
        #[must_use]
        #[inline(always)]
    ...
    impl defmt::Format for Hsclken {
        fn format(&self, f: defmt::Formatter) {
            defmt::write!(
                f, "Hsclken {{ hfxten: {=bool:?}, useexthfclk: {=bool:?} }}", self
                .hfxten(), self.useexthfclk()
            )
        }
    }
    ///SYSCTL interrupt index.
    ...
    impl defmt::Format for Int {
        fn format(&self, f: defmt::Formatter) {
            defmt::write!(
                f,
                "Int {{ lfoscgood: {=bool:?}, anaclkerr: {=bool:?}, flashsec: {=bool:?}, sramsec: {=bool:?}, lfxtgood: {=bool:?}, hfclkgood: {=bool:?}, hsclkgood: {=bool:?} }}",
                self.lfoscgood(), self.anaclkerr(), self.flashsec(), self.sramsec(), self
                .lfxtgood(), self.hfclkgood(), self.hsclkgood()
            )
        }
    }
    ///Low frequency crystal oscillator (LFXT) configuration.
    ...
    impl defmt::Format for Mclkcfg {
        fn format(&self, f: defmt::Formatter) {
            defmt::write!(
                f,
                "Mclkcfg {{ mdiv: {=u8:?}, flashwait: {:?}, usemftick: {=bool:?}, usehsclk: {=bool:?}, uselfclk: {=bool:?}, stopclkstby: {=bool:?}, mclkdeadchk: {=bool:?} }}",
                self.mdiv(), self.flashwait(), self.usemftick(), self.usehsclk(), self
                .uselfclk(), self.stopclkstby(), self.mclkdeadchk()
            )
        }
    }
    ///NMI interrupt clear.
    ...
    impl Nmi {
        ...
        ///VBAT Power Off.
        #[inline(always)]
        pub const fn set_vbatdn(&mut self, val: bool) {
            self.0 = (self.0 & !(0x01 << 5usize)) | (((val as u32) & 0x01) << 5usize);
        }
        ///VBAT Power On.
        #[must_use]
        ...
        ///VBAT Power On.
        #[inline(always)]
        pub const fn set_vbatup(&mut self, val: bool) {
            self.0 = (self.0 & !(0x01 << 6usize)) | (((val as u32) & 0x01) << 6usize);
        }
    }
    ...
    impl core::fmt::Debug for Nmi {
        fn fmt(&self, f: &mut core::fmt::Formatter) -> core::fmt::Result {
            f.debug_struct("Nmi")
                ...
                .field("vbatdn", &self.vbatdn())
                .field("vbatup", &self.vbatup())
                .finish()
        }
    }
    #[cfg(feature = "defmt")]
    impl defmt::Format for Nmi {
        fn format(&self, f: defmt::Formatter) {
            defmt::write!(
                f,
                "Nmi {{ borlvl: {=bool:?}, wwdt0: {=bool:?}, lfclkfail: {=bool:?}, flashded: {=bool:?}, sramded: {=bool:?}, vbatdn: {=bool:?}, vbatup: {=bool:?} }}",
                self.borlvl(), self.wwdt0(), self.lfclkfail(), self.flashded(), self
                .sramded(), self.vbatdn(), self.vbatup()
            )
        }
    }
    ///NMI interrupt index.
    ...
    impl Sysstatus {
        ...
        ///VBATGOOD is set by hardware when the VBAT Power Domain is valid.
        #[must_use]
        #[inline(always)]
        ...
        ///VBATGOOD is set by hardware when the VBAT Power Domain is valid.
        #[inline(always)]
        pub const fn set_vbatgood(&mut self, val: bool) {
            self.0 = (self.0 & !(0x01 << 7usize)) | (((val as u32) & 0x01) << 7usize);
        }
    ...
    impl defmt::Format for Sysstatus {
        fn format(&self, f: defmt::Formatter) {
            defmt::write!(
                f,
                "Sysstatus {{ flashded: {=bool:?}, flashsec: {=bool:?}, borcurthreshold: {:?}, borlvl: {=bool:?}, anacpumpgood: {=bool:?}, pmuirefgood: {=bool:?}, vbatgood: {=bool:?}, extrstpindis: {=bool:?}, swdcfgdis: {=bool:?}, shdniolock: {=bool:?}, rebootattempts: {=u8:?} }}",
                self.flashded(), self.flashsec(), self.borcurthreshold(), self.borlvl(),
                self.anacpumpgood(), self.pmuirefgood(), self.vbatgood(), self
    ...
    impl Systemcfg {
        ...
        ///SUPERCAP specifies whether the battery backup system can be powered by a SUPERCAP.
        #[must_use]
        #[inline(always)]
        ...
        ///SUPERCAP specifies whether the battery backup system can be powered by a SUPERCAP.
        #[inline(always)]
        pub const fn set_supercapen(&mut self, val: bool) {
            self.0 = (self.0 & !(0x01 << 8usize)) | (((val as u32) & 0x01) << 8usize);
        }
    }
    ...
    impl core::fmt::Debug for Systemcfg {
        fn fmt(&self, f: &mut core::fmt::Formatter) -> core::fmt::Result {
            f.debug_struct("Systemcfg")
                ...
                .field("supercapen", &self.supercapen())
                .finish()
        }
    }
    #[cfg(feature = "defmt")]
    impl defmt::Format for Systemcfg {
        fn format(&self, f: defmt::Formatter) {
            defmt::write!(
                f,
                "Systemcfg {{ wwdtlp0rstdis: {=bool:?}, flasheccrstdis: {=bool:?}, supercapen: {=bool:?} }}",
                self.wwdtlp0rstdis(), self.flasheccrstdis(), self.supercapen()
            )
        }
    }
    ///SYSCTL register write lockout.
...
pub mod vals {
    ...
    #[cfg_attr(feature = "defmt", derive(defmt::Format))]
    pub enum Hsclksel {
        ///The SYSPLL position of the mux, and the reset value. This device has no SYSPLL, so HSCLK has no source while this is selected.
        Syspll = 0x0,
