...
pub mod regs {
    ...
    impl defmt::Format for Genclkcfg {
        fn format(&self, f: defmt::Formatter) {
            defmt :: write ! (f , "Genclkcfg {{ exclksrc: {:?}, exclkdivval: {:?}, exclkdiven: {=bool:?}, mfpclksrc: {:?}, hfclk4mfpclkdiv: {:?}, fccselclk: {:?}, fcctrigsrc: {:?}, fcclvltrig: {:?}, anacpumpcfg: {:?}, fcctrigcnt: {=u8:?} }}" , self . exclksrc () , self . exclkdivval () , self . exclkdiven () , self . mfpclksrc () , self . hfclk4mfpclkdiv () , self . fccselclk () , self . fcctrigsrc () , self . fcclvltrig () , self . anacpumpcfg () , self . fcctrigcnt ())
        }
    }
    #[doc = "General clock enable control."]
    #[repr(transparent)]
    ...
    impl Hsclkcfg {
        ...
        #[doc = "HSCLKSEL selects the HSCLK source (SYSPLL or HFCLK)."]
        #[inline(always)]
        pub const fn set_hsclksel(&mut self, val: bool) {
            self.0 = (self.0 & !(0x01 << 0usize)) | (((val as u32) & 0x01) << 0usize);
        }
    }
    ...
    impl defmt::Format for Hsclkcfg {
        fn format(&self, f: defmt::Formatter) {
            defmt::write!(f, "Hsclkcfg {{ hsclksel: {=bool:?} }}", self.hsclksel())
        }
    }
    #[doc = "High-speed clock (HSCLK) source enable/disable."]
    #[repr(transparent)]
    ...
    impl defmt::Format for Int {
        fn format(&self, f: defmt::Formatter) {
            defmt :: write ! (f , "Int {{ lfoscgood: {=bool:?}, anaclkerr: {=bool:?}, flashsec: {=bool:?}, sramsec: {=bool:?}, lfxtgood: {=bool:?}, hfclkgood: {=bool:?}, hsclkgood: {=bool:?} }}" , self . lfoscgood () , self . anaclkerr () , self . flashsec () , self . sramsec () , self . lfxtgood () , self . hfclkgood () , self . hsclkgood ())
        }
    }
    #[doc = "Low frequency crystal oscillator (LFXT) configuration."]
    #[repr(transparent)]
    ...
    impl defmt::Format for Nmi {
        fn format(&self, f: defmt::Formatter) {
            defmt :: write ! (f , "Nmi {{ borlvl: {=bool:?}, wwdt0: {=bool:?}, lfclkfail: {=bool:?}, flashded: {=bool:?}, sramded: {=bool:?}, vbatdn: {=bool:?}, vbatup: {=bool:?} }}" , self . borlvl () , self . wwdt0 () , self . lfclkfail () , self . flashded () , self . sramded () , self . vbatdn () , self . vbatup ())
        }
    }
    #[doc = "NMI interrupt index."]
    #[repr(transparent)]
    ...
    impl defmt::Format for Sysstatus {
        fn format(&self, f: defmt::Formatter) {
            defmt :: write ! (f , "Sysstatus {{ flashded: {=bool:?}, flashsec: {=bool:?}, borcurthreshold: {:?}, borlvl: {=bool:?}, anacpumpgood: {=bool:?}, pmuirefgood: {=bool:?}, vbatgood: {=bool:?}, extrstpindis: {=bool:?}, swdcfgdis: {=bool:?}, shdniolock: {=bool:?}, rebootattempts: {=u8:?} }}" , self . flashded () , self . flashsec () , self . borcurthreshold () , self . borlvl () , self . anacpumpgood () , self . pmuirefgood () , self . vbatgood () , self . extrstpindis () , self . swdcfgdis () , self . shdniolock () , self . rebootattempts ())
        }
    }
    #[doc = "Clear sticky bits of SYSSTATUS."]
    #[repr(transparent)]
    ...
    impl defmt::Format for Systemcfg {
        fn format(&self, f: defmt::Formatter) {
            defmt :: write ! (f , "Systemcfg {{ wwdtlp0rstdis: {=bool:?}, flasheccrstdis: {=bool:?}, supercapen: {=bool:?} }}" , self . wwdtlp0rstdis () , self . flasheccrstdis () , self . supercapen ())
        }
    }
    #[doc = "SYSCTL register write lockout."]
    #[repr(transparent)]
