...
pub mod regs {
    ...
    impl Ackctl {
        ///I2C Target ACK Override Enable Read SR.ACKOEN to check current status of this bit.
        #[must_use]
        #[inline(always)]
        ...
        ///ACK Override Value Note: for General Call this bit will be ignored if set to NACK and Target continues to receive data.
        #[must_use]
        #[inline(always)]
        ...
        ///When set this bit will automatically enable target override acknowlede following a Start Condition. Status bit ACKOEN will reflect a value of '1' when automatic override enable is used.
        #[must_use]
        #[inline(always)]
        ...
        ///When set this bit will automatically turn on the Target acknowledge override following a ACK/NACK of the byte received just prior to the PEC byte. However, setting ACKCTL.ACKOEN bit will not automatically be ACKed/NACKed by the State Machine and firmware must perform this function by writing Target.ACKCTL register. Status bit ACKOEN will reflect a value of '1' when automatic override enable is used.
        #[must_use]
        #[inline(always)]
        ...
        ///When set, this bit will automatically turn on target acknowledge enable field following the ACK/NACK of the received PEC byte. Status bit ACKOEN will reflect a value of '1' when automatic override enable is used.
        #[must_use]
        #[inline(always)]
    ...
    impl defmt::Format for Ackctl {
        fn format(&self, f: defmt::Formatter) {
            defmt::write!(
                f,
                "Ackctl {{ ackoen: {=bool:?}, ackoval: {=bool:?}, ackoen_on_start: {=bool:?}, ackoen_on_pecnext: {=bool:?}, ackoen_on_pecdone: {=bool:?} }}",
    ...
    impl CpuInt {
        ///Receive Data Interrupt Signals that a byte has been received.
        #[must_use]
        #[inline(always)]
        ...
        ///RXFIFO full event. This interrupt is set if an Target RX FIFO is full.
        #[must_use]
        #[inline(always)]
        ...
        ///Target Transmit FIFO Empty interrupt mask. This interrupt is set if all data in the Transmit FIFO have been shifted out and the transmit goes into idle mode.
        #[must_use]
        #[inline(always)]
    ...
    impl defmt::Format for CpuInt {
        fn format(&self, f: defmt::Formatter) {
            defmt::write!(
                f,
                "CpuInt {{ rxdone: {=bool:?}, txdone: {=bool:?}, rxtrg: {=bool:?}, txtrg: {=bool:?}, rxfull: {=bool:?}, txempty: {=bool:?}, tx_unfl: {=bool:?}, rx_ovfl: {=bool:?}, gencall: {=bool:?}, start: {=bool:?}, stop: {=bool:?}, pec_rx_err: {=bool:?}, timeouta: {=bool:?}, timeoutb: {=bool:?}, dma_done_rx: {=bool:?}, dma_done_tx: {=bool:?}, arblost: {=bool:?} }}",
                self.rxdone(), self.txdone(), self.rxtrg(), self.txtrg(), self.rxfull(),
                self.txempty(), self.tx_unfl(), self.rx_ovfl(), self.gencall(), self
                .start(), self.stop(), self.pec_rx_err(), self.timeouta(), self
                .timeoutb(), self.dma_done_rx(), self.dma_done_tx(), self.arblost()
            )
        }
    }
    ///I2C Target Control Register.
    ...
    impl defmt::Format for Ctr {
        fn format(&self, f: defmt::Formatter) {
            defmt::write!(
                f,
                "Ctr {{ enable: {=bool:?}, gencall: {=bool:?}, txempty_on_treq: {=bool:?}, txtrig_txmode: {=bool:?}, txwait_stale_txfifo: {=bool:?}, rxfull_on_rreq: {=bool:?}, en_defhostadr: {=bool:?}, en_alrespadr: {=bool:?}, en_defdevadr: {=bool:?}, suspend: {=bool:?}, clkstretch: {=bool:?}, wuen: {=bool:?} }}",
                self.enable(), self.gencall(), self.txempty_on_treq(), self
                .txtrig_txmode(), self.txwait_stale_txfifo(), self.rxfull_on_rreq(), self
                .en_defhostadr(), self.en_alrespadr(), self.en_defdevadr(), self
                .suspend(), self.clkstretch(), self.wuen()
            )
        }
    }
    ///Interrupt mask.
    ...
    impl Oar {
        ///Own Address: This field specifies bits A9 through A0 of the Target address. In 7-bit addressing mode as selected by I2CSOAR.MODE bit, the top 3 bits are don't care.
        #[must_use]
        #[inline(always)]
    ...
    impl defmt::Format for Oar {
        fn format(&self, f: defmt::Formatter) {
            defmt::write!(
                f, "Oar {{ oar: {=u16:?}, oaren: {=bool:?}, mode: {:?} }}", self.oar(),
                self.oaren(), self.mode()
            )
        }
    }
    ///Own Address 2.
    ...
    impl Oar2 {
        ///Own Address 2This field specifies the alternate OAR2 address.
        #[must_use]
        #[inline(always)]
        ...
        ///Own Address 2 Mask: This field specifies bits A6 through A0 of the Target address. The bits with value 1 in SOAR2.OAR2_MASK field will make the corresponding incoming address bits to match by default regardless of the value inside SOAR2.OAR2 i.e. corresponding SOAR2.OAR2 bit is a dont care.
        #[must_use]
        #[inline(always)]
    ...
    impl defmt::Format for Oar2 {
        fn format(&self, f: defmt::Formatter) {
            defmt::write!(
                f, "Oar2 {{ oar2: {=u8:?}, oar2en: {=bool:?}, oar2_mask: {=u8:?} }}",
...
is set to 'STOP'.*/
    ...
    impl Pecsr {
        ///This is the current PEC Byte Count of the State Machine.
        #[must_use]
        #[inline(always)]
    ...
    impl Sr {
        ...
        pub const fn set_addrmatch(&mut self, val: u16) {
            self.0 = (self.0 & !(0x03ff << 16usize))
                | (((val as u32) & 0x03ff) << 16usize);
        }
    }
    impl Default for Sr {
        #[inline(always)]
    ...
    impl defmt::Format for Sr {
        fn format(&self, f: defmt::Formatter) {
            defmt::write!(
                f,
                "Sr {{ rreq: {=bool:?}, treq: {=bool:?}, rxmode: {=bool:?}, oar2sel: {=bool:?}, qcmdst: {=bool:?}, qcmdrw: {=bool:?}, busbsy: {=bool:?}, txmode: {=bool:?}, stale_txfifo: {=bool:?}, rxclr: {=bool:?}, txclr: {=bool:?}, rxfe: {=bool:?}, rxff: {=bool:?}, txfe: {=bool:?}, txff: {=bool:?}, ackoen: {=bool:?}, addrmatch: {=u16:?} }}",
                self.rreq(), self.treq(), self.rxmode(), self.oar2sel(), self.qcmdst(),
                self.qcmdrw(), self.busbsy(), self.txmode(), self.stale_txfifo(), self
                .rxclr(), self.txclr(), self.rxfe(), self.rxff(), self.txfe(), self
                .txff(), self.ackoen(), self.addrmatch()
            )
        }
    }
    ///I2C Timeout Count Register.
    ...
    impl core::fmt::Debug for Ttxdata {
        fn fmt(&self, f: &mut core::fmt::Formatter) -> core::fmt::Result {
            f.debug_struct("Ttxdata").field("data", &self.data()).finish()
        }
    }
    #[cfg(feature = "defmt")]
