...
pub mod regs {
    ...
    impl defmt::Format for CpuInt {
        fn format(&self, f: defmt::Formatter) {
            defmt :: write ! (f , "CpuInt {{ rtout: {=bool:?}, frmerr: {=bool:?}, parerr: {=bool:?}, brkerr: {=bool:?}, ovrerr: {=bool:?}, rxne: {=bool:?}, rxpe: {=bool:?}, linc[0]: {=bool:?}, linc[1]: {=bool:?}, linovf: {=bool:?}, rxint: {=bool:?}, txint: {=bool:?}, eot: {=bool:?}, addr_match: {=bool:?}, cts: {=bool:?}, dma_done_rx: {=bool:?}, dma_done_tx: {=bool:?}, nerr: {=bool:?} }}" , self . rtout () , self . frmerr () , self . parerr () , self . brkerr () , self . ovrerr () , self . rxne () , self . rxpe () , self . linc (0usize) , self . linc (1usize) , self . linovf () , self . rxint () , self . txint () , self . eot () , self . addr_match () , self . cts () , self . dma_done_rx () , self . dma_done_tx () , self . nerr ())
        }
    }
    #[doc = "Interrupt index."]
    #[repr(transparent)]
